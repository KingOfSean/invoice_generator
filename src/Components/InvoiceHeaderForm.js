    ...
    return (
        <div className='container'>
                            ...
                            handleCardContent() ?   
                                <div className='invoice-address-card'>
                                    <div className='invoice-address-container'>
                                        ...
                                        <span className='invoice-address-top-line'>{`${usersAddressStreet} ${usersAddressStreet2}`}</span>
                                        <span className='invoice-address-bottom-line'>{`${usersAddressCity}, ${usersAddressState} ${usersAddressZip}`}</span>
                                    </div>
                                    <div className='invoice-address-container'>
                                        <span className='invoice-address-header'>Bill To:</span>
                                        <span className='invoice-address-name'>{billToHeaderName}</span>
                                        <span className='invoice-address-top-line'>{`${billToAddressStreet} ${billToAddressStreet2}`}</span>
                                        <span className='invoice-address-bottom-line'>{`${billToAddressCity}, ${billToAddressState} ${billToAddressZip}`}</span>
                                    </div>
                                </div>
                            : 
                                <div className='centered-flex-row'>
                                    <span className='invoice-card-default-text'>No header content to display</span>
                                </div>
                        }
                    </CardContent>
                </Card>
                <InvoiceHeaderDateNameTable invoiceName={invoiceName} invoiceDate={invoiceDate} />
            </div>
            <button 
            ...
            <CustomDialog
                ...
                content={
                    <InvoiceHeaderAddressFormTabs setHeaderAddressDialog={setHeaderAddressDialog} />
                }
            />
        </div>
    );
